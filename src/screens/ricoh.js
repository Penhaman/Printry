...
import { listRicoh } from './lists/RicohList';
import { Searchbar } from 'react-native-paper';
import {AdMobBanner} from 'expo-ads-admob';

export default function Ricoh ({navigation}) {
    const [search, setSearch] = React.useState('');
    const filteredSearch = listRicoh.filter(model => {
    ...
    return (
        <View style={globalStyles.container}>
            ...
            <FlatList data={filteredSearch} keyExtractor={(item) => item.key} renderItem={({item}) => (
                <TouchableOpacity onPress={() => navigation.navigate('errorRicoh', {
